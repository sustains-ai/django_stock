...
def get_portfolio_returns_df(portfolio: Portfolio) -> pd.DataFrame:
    ...
    data = HistoricalStockData.objects.filter(portfolio=portfolio).order_by("date")
    ...
    for symbol in symbols:
        symbol_data = data.filter(symbol=symbol).values("date", "adjusted_close")
        if symbol_data:
...
def portfolio_risk_agent(portfolio_id: int, question: str) -> str:
    try:
        ...
        risk_measures = calculate_risk_measures(returns_df, list(returns_df.columns))
